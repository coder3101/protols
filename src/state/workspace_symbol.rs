...
mod test {
    ...
    fn test_workspace_symbols() {
        ...
        let ipath = vec![current_dir.join("src/state/input")];
        let base_uri_str = Url::from_directory_path(&current_dir)
            .unwrap()
            .to_string()
            .trim_end_matches('/')
            .to_string();
        let a_uri = Url::from_file_path(current_dir.join("src/state/input/a.proto")).unwrap();
        let b_uri = Url::from_file_path(current_dir.join("src/state/input/b.proto")).unwrap();
        let c_uri = Url::from_file_path(current_dir.join("src/state/input/c.proto")).unwrap();
        ...
        assert_yaml_snapshot!(all_symbols, { "[].location.uri" => insta::dynamic_redaction(move |c, _| {
            ...
            format!("file://<redacted>/src/state/input/{file_name}")
